...
float PID::compute_func (float qn, int cn, float rho, float d_av[], float d[], int node, float y[] )
{
...
float PID::consensus_function(float dn[], int node, float cn, float qn, float Kn[], float on, float Ln)
{
  float rho = 0.01;
  int iter = 1; int i;
  ...
  Serial.print("d_best ");
  Serial.println(d_best[node]);
  return d_best[node];

}

float PID::control_signal(float ref, float LUX, float dn[], int node, float cn, float qn, float Kn[], float on, float Ln)
{
  // defines error
  e = ref - LUX;

  Serial.print("ref_change=");
  Serial.println(ref_change);
  Serial.print("uff=");
  Serial.println(uff);

  if (ref_change == 1) {
    uff = consensus_function(dn, node, cn, qn, Kn, on, Ln);
    uff = uff / 100 * 255;
    Serial.print("uff=");
    Serial.println(uff);
    ref_change = 0;
